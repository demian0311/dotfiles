...
🗑 **A second, near-identical service was retired on 2026-09-04** —
`anchor-api-reference.service`, on local 4350 and public 14350, serving a
*pre-generated* copy of the same page out of `~/anchor-hub/api-reference/`. Its
own header said why it existed: `hub.mjs` was being edited live and in place,
with no version control, at the moment it was written. That reason is gone —
this file is in the repo now — and the two were never equivalent anyway: the
static build had to be regenerated by hand, while `/api-docs` reads the spec
from the running Worker on every load and cannot go stale. The unit, its
`tailscale serve` mapping, `api-reference-server.mjs`, `build-api-reference.sh`
and the generated directory are all gone.

### Theming it

🔴 **The theme is read out of the page's own CSS variables at init, not written
again as a JS object.** Redoc wants hex strings and the palette lives in
`SLATE` as custom properties, so the obvious move is a second copy in
JavaScript — and a second copy of a palette is a second thing to forget when
the first one changes. `getComputedStyle` already resolves the media query, so
light and dark both fall out of the block that is already there.

⚠️ **Every theme key came from Redoc 2.5.0's own `src/theme.ts`**, fetched on
2026-09-04, not from memory. Redoc drops a key it does not know **silently**,
so a misremembered name reads as "the theme did not apply" rather than as an
error.

Two things Redoc gives no theme key for, both fixed in CSS against the element
rather than the class — its class names are emotion-generated (`sc-dAlyuH`) and
change between builds:

- **It shouts its section labels.** `sidebar.groupItems.textTransform: 'none'`
  handles the nav, but the `h5` labels (Authorizations, query Parameters,
  Response schema) have no key. Their source text is mid-sentence, so removing
  the caps alone would leave a lowercase heading — hence the `::first-letter`
  rule alongside. **The `GET` / `POST` badges stay uppercase deliberately**:
  those are the literal tokens sent on the wire, which is the value carve-out
  in the no-caps rule, not a heading.
- **It hard-codes those same labels at `rgba(38, 50, 56, .5)`.** Measured
  2026-09-04: **2.61:1** on the light ground, and on the dark one it is a
  near-black at half alpha — all but invisible. They are `textMuted` now,
  5.36:1 and 6.91:1.

`--panel` / `--panel-ink` exist for this page alone. Redoc's right-hand sample
panel is dark in both of its own themes by design, so it needs a colour that
does not follow the page ground; in dark mode it takes `surface`, one step
above the ground, or it stops reading as a panel at all.
